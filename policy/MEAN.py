...
class MEANPolicy(nn.Module):
    ...
    def __init__(self, config: Optional[MEANConfig] = None, **kwargs):
        """
        Initialize IQN policy network

        Args:
            config: IQN configuration object
        ...
        # mean embedding, ablation study, with more layers to compare
        self.mean_embedding_mlp = nn.Sequential(
    ...
    def encode_entities(self, obs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        mean_embedding = torch.stack(list(mean_feature_per_type.values()), dim=1)  # [B, 4, H]
        mean_embedding = mean_embedding.mean(dim=1)  # [B, H]

        mean_embedding = self.mean_embedding_mlp(mean_embedding)  # [B, 2H]
