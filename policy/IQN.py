...
@dataclass
class IQNConfig:
    """Configuration class for IQN policy network"""
    hidden_dim: int = 256
    num_heads: int = 8
    num_layers: int = 3
    action_size: int = 9
...
class IQNPolicy(nn.Module):
    """
    Implicit Quantile Networks (IQN) policy network implementation
    Uses Transformer architecture to process multi-entity inputs and IQN method to estimate action value distribution
    """

    def __init__(self, config: Optional[IQNConfig] = None, **kwargs):
        """
